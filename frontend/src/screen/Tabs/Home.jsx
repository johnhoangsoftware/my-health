    ...
    return (
      <View className="bg-white rounded-xl w-40 h-full mr-4 mb-3">
        ...
        <View className="px-2 py-1 space-y-1 mb-3">
          <Text className="font-semibold h-9" numberOfLines={2}>
            {props.name}
          </Text>
          <Text className="text-gray-500 text-xs h-14" numberOfLines={3}>
            {props.address}
          </Text>
          <TouchableOpacity
            ...
            onPress={() => {
              navigation.navigate("Chọn hồ sơ", { hospital: props });
            }}
          >
            <Text className="font-semibold text-base text-center text-white p-1">
              Đặt khám
            </Text>
          </TouchableOpacity>
        </View>
      ...
      .then((hs) => {
        setHospitalList(
          hs.map((i) => ({
            ...
            imageURL:
              i.avatar ||
              "https://insmart.com.vn/wp-content/uploads/2021/05/BV-108-2.jpg",
          }))
        );
      })
      .catch((err) => {
    ...
    <>
      ...
      <ScrollView horizontal={false} showsVerticalScrollIndicator={true}>
        ...
        >
          ...
          <View className="h-3/5">{itemList}</View>
        </LinearGradient>
      </ScrollView>
    </>
  );
